############################################################################################################# 9.
##### 9. Reduced ML modelling with/without no covariates (Table 2; Fig.3; Table S12,13,15,16; Fig.S13&14)
setwd("~/Desktop") 
...
############################################################################################################# 9.1
##################### 9.1 Reduced ML models (no covariates) ####################
df <- df %>% 
...
#### Table S12&13; and Fig.S13. To generate Table S15&16 and Fig.S14, simply including 
...
results_list <- foreach(model_name = names(model_list), 
  ...
  for (r in 1:repeats_outer) {
    ...
    for (f in seq_along(outer_folds)) {
      ...
      # train_data$Noise <- rnorm(nrow(train_data)) # Small noise for single-models
      # test_data$Noise  <- rnorm(nrow(test_data)) # Small noise for single-models
...
# all_avg[1, 3] <- 0.009949075 # GLMNET model 1a, test
# all_avg[4, 3] <- 0.004147951 # GLMNET model 1a, train
...
ggplot(all_hyper_metric, aes(x = lambda, y = alpha, color = R2)) +
  ...
  theme(
    ...
    strip.text = element_text(size = 6)) # Fig.S13 or Fig.S14
...
############################################################################################################# 9.2.4
##### 9.2.4 Data extractions - p values b/t models on test sets - Table 2,S13&15 #####
df_test <- all_metrics %>%
  filter(Set == "Test") %>% 
  select(RMSE, Repeat, Fold, Model) %>% # Change R2/RMSE/MAE
  pivot_wider(names_from = Model, values_from = RMSE) %>% # Change R2/RMSE/MAE
...
long_df_T1_3 <- df_test %>% 
  pivot_longer(cols = c("model3a","model3b","model3c","model3d","model3e","model3f",
                        ...
                        "model1.1","model2"),
               names_to = "Model", values_to = "RMSE") # Change R2/RMSE/MAE

## Pairwise Wilcoxon signed-rank test
p_value <- pairwise.wilcox.test(long_df_T1_3$RMSE,  # Change R2/RMSE/MAE
...
############################################################################################################# 9.3.2
################# 9.3.2 Calibration plots - GLM - Fig.S13&14 ###################
...
calibration_data_fig.S13 <- all_calibration_data %>%
  ...
  mutate(Model = forcats::fct_relevel(Model, "model1", "model1.1", "model2",
                                      "model3","model3.1","model3.2","model3.3","model3.4","model3.5")) # Fig.S13
labels_fig.S13 <- c(
  model1 = "4-Metabo.",
  model1.1 = "4-Metabo. + Albumin",
  model2 = "2-Clinc.",
...
metrics <- df_final%>% # df_final is in 9.3.1 - Step 6; for Fig.S13&14
...
metrics <- metrics %>% # For Fig.S13&14
...
ggplot(calibration_data_fig.S13, aes(x = Predicted, y = Actual)) + 
  ...
  theme(
    ...
    legend.text = element_text()) # Fig.S13&14

## Residual plots for all models  # Fig.S13&14
...
ggplot(all_residual_data, aes(x = Predicted, y = Residual)) +
  ...
  theme(
    ...
    strip.text = element_text(family = "Times New Roman", size = 8)) # Fig.S13&14
