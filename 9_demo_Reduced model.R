############################################################################################################# 9.
##### 9. ML modelling (Table 4; Fig.2; Table S11; Fig.S13)
setwd("~/Desktop")
...
############################################################################################################# 9.2.4
##### 9.2.4 Data extractions - p values b/t models on test sets - Table 4&S11 #####
df_test <- all_metrics %>%
  filter(Set == "Test") %>% 
...
p_value <- pairwise.wilcox.test(long_df_T1_3$MAE,  # Change R2/RMSE/MAE
                     long_df_T1_3$Model, paired = TRUE, p.adjust.method = "none")  # Table 4&S11
view(p_value$p.value)

############################################################################################################# 9.3.1
##################### 9.3.1 Bar plots Fig. 2 & S13 - GLM #######################
labels_fig.2 <- c(
  model1 = "4-Metabo.",
  model1.1 = "4-Metabo. + Albumin",
  model2 = "2-Clinc.",
...
plot_metric <- function(metric_name, y_label_expr) {
  plot_data %>%
    ...
    scale_x_discrete(labels = labels_fig.2) + # Fig.2&S13 only
    geom_bar_pattern(
      stat = "identity",
