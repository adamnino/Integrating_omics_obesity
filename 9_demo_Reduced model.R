############################################################################################################# 9.
##### 9. ML modelling (Table 2; Fig.2; Table S12&13; Fig.S13)
setwd("~/Desktop")
...
ggplot(all_hyper_metric, aes(x = lambda, y = alpha, color = R2)) +
  ...
  theme(
    ...
    strip.text = element_text(size = 6)) # Fig.S13
...
############################################################################################################# 9.2.4
##### 9.2.4 Data extractions - p values b/t models on test sets - Table 2&S13 #####
df_test <- all_metrics %>%
  filter(Set == "Test") %>% 
...
p_value <- pairwise.wilcox.test(long_df_T1_3$MAE,  # Change R2/RMSE/MAE
                     long_df_T1_3$Model, paired = TRUE, p.adjust.method = "none")  # Table 2&S13
view(p_value$p.value)

############################################################################################################# 9.3.1
######################## 9.3.1 Bar plots Fig.2 - GLM ###########################
labels_fig.2 <- c(
  model1 = "4-Metabo.",
  model1.1 = "4-Metabo. + Albumin",
  model2 = "2-Clinc.",
...
plot_metric <- function(metric_name, y_label_expr) {
  plot_data %>%
    ...
    geom_text(aes(label = sprintf("%.3f", Mean), y = CI_Upper + 0.01), size = 3.5, vjust = -0.5) +
...
############################################################################################################# 9.3.2
################### 9.3.2 Calibration plots - GLM - Fig.S13 ####################
...
metrics <- df_final%>% # df_final is in 9.3.1 - Step 6; for Fig.S13
...
ggplot(calibration_data_fig.S13, aes(x = Predicted, y = Actual)) + 
  ...
  theme(
    ...
    legend.text = element_text()) # Fig.S13

## Residual plots for all models  # Fig.S13
...
ggplot(all_residual_data, aes(x = Predicted, y = Residual)) +
  ...
  theme(
    ...
    strip.text = element_text(family = "Times New Roman", size = 8)) # Fig.S13
